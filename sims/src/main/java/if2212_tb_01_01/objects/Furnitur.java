...
public class Furnitur extends Objek {
    ...
    public Furnitur(String nama, String aksi, Integer harga, Integer panjang, Integer lebar, Point posisi,
            boolean isVertikal) {
        super(nama, "peralatan");
        this.aksi = aksi;
        this.harga = harga;
    ...
    public static void printListFurnitur() {
        ...
        for (Furnitur x : listFurnitur) {
            System.out.println(i + ". " + x.getNama() + " (Aksi: " + x.getAksi() + " ,Harga: " + x.getHarga()
                    + " ,Dimensi: " + x.getPanjang() + "x" + x.getLebar() + ")");
            i++;
        }
    }

    public String getInfo() {
        String ret = String.format("Nama: %s\nKategori: Peralatan\nAksi: %s\nHarga: %d\nDimensi: %dx%d", this.getNama(),
                aksi, harga, panjang, lebar);
        return (ret);
    }
